...
RequestHandler.prototype.DEFAULT_HTML_CONTENT_TYPE =
...
RequestHandler.prototype._assertResponse = function(response, allowBody,
  ...
  if (hop.call(response, 'stream')) {
    ...
    if (hop.call(response, 'html')) {
      throw new TypeError(
          'Unexpected `html` value when `stream` is present.');
    }
  ...
  if (hop.call(response, 'chunk')) {
    ...
    if (hop.call(response, 'html')) {
      throw new TypeError(
          'Unexpected `html` value when `chunk` is present.');
    }
    if (hop.call(response, 'json')) {
      throw new TypeError(
  ...
  if (hop.call(response, 'html')) {
    if (typeof response.html !== 'string' && !Array.isArray(response.html)) {
    ...
    if (hop.call(response, 'json')) {
      throw new TypeError(
          'Unexpected `json` value when `html` is present.');
    }
    if (hop.call(response, 'form')) {
...
RequestHandler.prototype._writeResponse = function(promise, state) {
  ...
  return promise.then(function(response) {
    ...
    if (response.stream) {
      ...
      res.setHeader('content-type', handler.DEFAULT_HTML_CONTENT_TYPE);
